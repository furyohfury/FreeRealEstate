...
namespace Gaskellgames.EditorOnly
{
    ...
    public class GaskellgamesHub : GgEditorWindow
    {
        ...
        private static readonly int windowHeight = 570;
        ...
        private void HandleGaskellgamesLogs()
        {
            float defaultWidth = EditorGUIUtility.labelWidth;
            EditorGUIUtility.labelWidth = settingsLabelWidth;
            GUIContent label = new GUIContent("Gaskellgames Logs", "Enable or Disable Gaskellgames specific logs.");
            GaskellgamesHubSettings_SO.ShowGaskellgamesLogs selected = (GaskellgamesHubSettings_SO.ShowGaskellgamesLogs)EditorGUILayout.EnumPopup(label, EditorWindowUtility.settings.ShowGgLogs, GUILayout.Width(settingsWidth));
            EditorGUIUtility.labelWidth = defaultWidth;
            if (EditorWindowUtility.settings.ShowGgLogs == selected) { return; }
            EditorWindowUtility.settings.ShowGgLogs = selected;
            EditorUtility.SetDirty(EditorWindowUtility.settings);
        }
