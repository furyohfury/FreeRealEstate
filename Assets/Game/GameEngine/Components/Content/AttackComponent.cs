...
{
	[Serializable]
	public sealed class AttackComponent : IComponent, IComponentInit, IComponentDispose // TODO separate on objects (not components)
	{
		public AndCondition CanAttack = new();
		...
		private void OnAttacked(Collider other)
		{
			if (other.gameObject.TryGetComponent(out Entity entity)
			    && entity.TryGetComponent(out LifeComponent lifeComponent))
			{
				lifeComponent.ChangeHealth(-_damage);
			}
		}
